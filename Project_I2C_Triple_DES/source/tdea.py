...
desMode = "td"
desRound = 1
inputBlock = "1234567890abcdef"
inputKey2 = "3b3898371520f75e"
inputKey = "8c1f609efca32a78"
...
if(desMode == "te"):
	for ae in range(0,16):
		...
		right = dataXdata
	outData = right+left
	print " First Encryption Output in Hex:\t\t%s " % toHex(outData)
	print " First Encryption OUtput in Binary:\t%s %s %s %s %s %s %s %s" % (outData[0:8], outData[8:16], outData[16:24], outData[24:32], outData[32:40], outData[40:48], outData[48:56], outData[56:64])
	right = outData[32:64]
	left = outData[0:32]
	for ae in range(15,-1,-1):
		expandData = expand(right)
		keyXdata = keyXor(roundKey2[ae],expandData)
		subData = substitute(keyXdata)
		permData = dataPerm(subData)
		dataXdata = dataXor(left,permData)
		left = right
		right = dataXdata
	outData = right+left
	print " Second Decryption Output in Hex:\t\t%s " % toHex(outData)
	print " Second Decryption Output in Binary:\t%s %s %s %s %s %s %s %s" % (outData[0:8], outData[8:16], outData[16:24], outData[24:32], outData[32:40], outData[40:48], outData[48:56], outData[56:64])
	right = outData[32:64]
	left = outData[0:32]
	for ae in range(0,16):
		expandData = expand(right)
		...
		right = dataXdata
	outData = right+left
	print " Third Encryption Output in Hex:\t\t%s " % toHex(outData)
	print " Third Encryption OUtput in Binary:\t%s %s %s %s %s %s %s %s" % (outData[0:8], outData[8:16], outData[16:24], outData[24:32], outData[32:40], outData[40:48], outData[48:56], outData[56:64])
